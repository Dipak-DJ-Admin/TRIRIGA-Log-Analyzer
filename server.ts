...
// Lazy initializer for Google GenAI SDK to prevent startup crashes if GEMINI_API_KEY is missing
let aiClient: GoogleGenAI | null = null;
function getGeminiClient(): GoogleGenAI | null {
  if (!aiClient) {
    const apiKey = process.env.GEMINI_API_KEY;
    if (!apiKey || apiKey === "MY_GEMINI_API_KEY") {
      console.warn("GEMINI_API_KEY environment variable is not defined or is placeholder. Using robust rule-based local analyzer.");
      return null;
    }
    try {
...
// Highly robust rule-based fallback analyzer for TRIRIGA Logs
function parseLogsLocally(logText: string, scenarioName?: string) {
  const text = logText.toLowerCase();
  
  // Extract CPU data
  let cpuMax = 45;
  const cpuMatches = logText.match(/cpu\s*[:=]\s*(\d+)%/i) || logText.match(/cpu\s+usage\s*[:=]?\s*(\d+(\.\d+)?)/i);
  if (cpuMatches) {
  ...
  if (text.includes("workflow execution failure") || text.includes("statetransitionexception") || text.includes("locktimeoutexception")) {
    totalWorkflows = 850;
    failedWorkflows = 38;
...
async function startServer() {
  ...
  // API endpoint to analyze log text dynamically using Gemini SDK or falling back to local parsed rules
  app.post("/api/analyze-logs", async (req, res) => {
    const { logText, scenarioName } = req.body;
    if (!logText || typeof logText !== "string") {
      res.status(400).json({ success: false, error: "Missing logText in request body" });
      return;
    }

    const ai = getGeminiClient();
    if (!ai) {
      const result = parseLogsLocally(logText, scenarioName);
      res.json({ ...result, isAI: false });
      return;
    }

    try {
      const prompt = `You are an expert IBM TRIRIGA Performance Engineer & Observability Copilot.
Analyze this log snippet or system dump from a TRIRIGA application server environment and identify bottlenecks, anomalies, and performance metrics.

Analyze based on these strict thresholds:
1. Sustained CPU above 80% is a "Critical System Constraint".
2. Gradual, non-recovering memory increases over time is a "Potential JVM Memory Leak".
3. Cache Miss Ratio exceeding 15% requires adjusting the TRIRIGA Object Cache settings in custom.properties.
4. Workflow Failure Rate exceeding 2% of total throughput is an "operational risk" due to locked business objects.

Please return a detailed JSON object fitting the following TypeScript structure:
{
  "status": "Healthy" | "Degraded" | "Critical",
  "executiveSummary": "A concise executive level summary of system status and performance impact",
  "detectedAnomalies": [
    {
      "title": "Anomaly Title",
  ...
  "metrics": {
    "cpuMax": number (max CPU usage observed, e.g. 85),
    "memoryTrend": "Stable" | "Upward" | "Downward" | "Fluctuating",
    "memoryLeakRisk": "Low" | "Medium" | "High",
    "cacheMissRatio": number (miss percentage, e.g. 18.5),
    "workflowFailureRate": number (failure rate percentage, e.g. 3.2),
    "totalWorkflowsProcessed": number,
    "totalWorkflowsFailed": number,
    "avgResponseTimeMs": number
  },
  "rca": "Detailed markdown explanation of root cause analysis and why this is happening.",
  "recommendations": [
    {
      "title": "Actionable Title",
      "category": "Property Edit" | "DB Query" | "Workflow Adjustment",
      "description": "Clear step-by-step description of how to resolve this specific issue based on TRIRIGA best practices",
      "codeSnippet": "Optional properties file configuration lines or SQL queries"
    }
  ]
}
...
Name of log scenario (for context): ${scenarioName || 'Custom Log Upload'}
`;
      ...
      const response = await ai.models.generateContent({
        ...
        config: {
          ...
          responseSchema: {
            type: Type.OBJECT,
            properties: {
              status: { type: Type.STRING, description: "System health status" },
              executiveSummary: { type: Type.STRING, description: "Short overview paragraph" },
              detectedAnomalies: {
                type: Type.ARRAY,
                items: {
                  type: Type.OBJECT,
              ...
              metrics: {
                type: Type.OBJECT,
                properties: {
                  cpuMax: { type: Type.NUMBER },
                  memoryTrend: { type: Type.STRING },
                  memoryLeakRisk: { type: Type.STRING },
                  cacheMissRatio: { type: Type.NUMBER },
                  workflowFailureRate: { type: Type.NUMBER },
                  totalWorkflowsProcessed: { type: Type.NUMBER },
                  totalWorkflowsFailed: { type: Type.NUMBER },
                  avgResponseTimeMs: { type: Type.NUMBER }
                },
                required: ["cpuMax", "memoryTrend", "memoryLeakRisk", "cacheMissRatio", "workflowFailureRate"]
              },
              rca: { type: Type.STRING, description: "Comprehensive Root Cause Analysis text in markdown" },
              recommendations: {
                type: Type.ARRAY,
                items: {
                  type: Type.OBJECT,
            ...
            required: ["status", "executiveSummary", "detectedAnomalies", "metrics", "rca", "recommendations"]
          }
        }
      });
      ...
      if (response.text) {
        ...
        res.json({ ...parsedData, success: true, isAI: true });
      } else {
        throw new Error("Empty response from Gemini API");
      }

    } catch (error) {
      console.error("Gemini analysis error, falling back to local parser:", error);
      const localResult = parseLogsLocally(logText, scenarioName);
      res.json({ ...localResult, success: true, isAI: false, error: "Fallback used due to AI parsing exception" });
    }
  });

  // Configure Vite integration for Full-Stack Hot-Reloading & Production Build
  ...
  app.listen(PORT, "0.0.0.0", () => {
    console.log(`TRIRIGA Observability Copilot server running on port ${PORT}`);
  });
}
